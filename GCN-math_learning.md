...
既然$Ax = \lambda x$的形式并不方便我们对特征值进行求解，数学家们提出了Schur分解定理[2],任意方阵A必然可以三角化$A = UTU^*$，其中T为上三角矩阵，U是一个么正矩阵。又由于A相似于T，两者拥有相同的特征值，而上三角矩阵的主对角元为其特征值，这样就方便我们获取。如果我们可以将矩阵$A = QR$那矩阵$B = RQ$两者相似，那我们只需要找到这样的一个分解算法了。


[2]:https://ccjou.wordpress.com/2009/07/10/%E7%9F%A9%E9%99%A3%E4%B8%89%E8%A7%92%E5%8C%96/
...
\end{aligned}
$$
在这里我们使用QR分解并不使用最简单的高斯分解法，最主要的原因在于分解精度和特定应用,比如求解最小平方近似解（当该情况下无解的时候）：
$$\min_x||Ax-b||^2$$

在熟悉完上面的QR分解方法之后，我们发现可以简化邻接矩阵的特征值求解，但是之前的分析是基于单个数值，那推导到一个feature上的时候这种方案是否同样适用呢。这时1931年的克雷诺夫（Aleksey Krylov）提出了Krylov子空间法：
